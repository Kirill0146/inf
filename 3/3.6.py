...
def final():
    ...
    def create(m, x, y, vx, vy, ax, ay, size, speed): #создает черепашку
        d = {'m':m, 'vx':vx, 'vy':vy, 'x':x, 'y':y, 'ax':ax, 'ay':ay, 'size':size, 'r':size * 10, 't':t.Turtle(shape = 'circle')}
        d['t'].shapesize(size)
        d['t'].penup()
        d['t'].speed(0)
        d['t'].goto(x, y)
    ...
    def move(): #сдвигает все черепашки
        for unit in pool:
            ...
            check_border(unit)
            if unit['size'] >= 1 and unit['size'] < 1.5:
                unit['t'].color('black')
            elif unit['size'] >= 1.5 and unit['size'] < 2.5:
                unit['t'].color('yellow')
            elif unit['size'] >= 2.5 and unit['size'] < 4.0:
                unit['t'].color('green')
            elif unit['size'] >= 4.0 and unit['size'] < 6.0:
                unit['t'].color('red')
            elif unit['size'] >= 6.0 and unit['size'] < 8.5:
                unit['t'].color('blue')
            elif unit['size'] >= 8.5:
                unit['t'].color('pink')
            ...
            for unit2 in pool:
                if unit1 != unit2:
                    ...
                    if check(unit1, unit2) == 1:
                        if unit1['r'] > unit2['r']:
                            unit1['r'], unit2['r'] = (unit1['r'] ** 2 + unit2['r'] ** 2) ** 0.5, 0
                            unit2['t'].color('white')
                            unit1['size'] = unit1['size'] + unit2['size']
                            unit1['vx'] = (unit1['m'] * unit1['vx'] + unit2['m'] * unit2['vx']) / (unit1['m'] + unit2['m'])
                            unit1['m'] = unit1['m'] + unit2['m']
                            pool.remove(unit2)
                            unit1['t'].shapesize(unit1['r'] / 10)
                        else:
                            unit2['r'], unit1['r'] = (unit1['r'] ** 2 + unit2['r'] ** 2) ** 0.5, 0
                            unit1['t'].color('white')
                            unit2['size'] = unit1['size'] + unit2['size']
                            unit2['vx'] = (unit1['m'] * unit1['vx'] + unit2['m'] * unit2['vx']) / (unit1['m'] + unit2['m'])
                            unit1['m'] = unit1['m'] + unit2['m']
                            pool.remove(unit1)
                            unit2['t'].shapesize(unit2['r'] / 10)
